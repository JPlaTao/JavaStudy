        ...
        try (FileInputStream fis = new FileInputStream(sourFile)) {
            while ((size = fis.read(byteSize)) != -1) {
                try (FileOutputStream fos =
                             new FileOutputStream(tarFile.getPath() +
                                     File.separator + name++)) {
...
//        int name = 0;
        ...
        try (FileInputStream fis = new FileInputStream(sourFile)) {
            while (size != -1) {
                File splitFile = new File(tarFile +
                        File.separator + System.currentTimeMillis());
                try (FileOutputStream fos =
                             new FileOutputStream(splitFile, true)) {
                    while (splitFile.length() < len) {
                        size = fis.read(byteSize);
                        if (size < 0)
                            break;
                        fos.write(byteSize, 0, size);
                    }
